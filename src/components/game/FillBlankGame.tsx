  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="text-center mb-8">
        <h2 className="text-2xl font-bold mb-2">{t('fill.title')}</h2>
        <p className="text-muted-foreground text-lg">
          {t('fill.question')} {currentIndex + 1} {t('fill.of')} {questions.length}
        </p>
            ...
            <CardContent className="p-6">
              <div className="text-xl font-medium mb-8 text-center leading-relaxed bg-muted/50 p-6 rounded-xl">
                ...
                <span className={`mx-2 px-4 py-1 rounded-lg font-bold ${
                  showResult ? isCorrect ? 'bg-success text-success-foreground' : 'bg-destructive text-destructive-foreground'
                  ...
                  <motion.div key={index} whileHover={{ scale: showResult ? 1 : 1.02 }} whileTap={{ scale: 0.98 }}>
                    <Button
                      ...
                      className={`w-full py-6 text-lg transition-all ${
                        showResult
                          ? index === currentQuestion.correctIndex ? 'bg-success/20 border-success'
