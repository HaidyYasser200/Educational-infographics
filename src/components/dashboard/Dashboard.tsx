...
export const Dashboard = ({ onBack }: DashboardProps) => {
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      const [progressRes, emotionsRes, profileRes] = await Promise.all([
        getProgress(),
        getEmotionLogs(),
        getProfile()
      ]);
  ...
  const stageScores = stages.map(stage => {
    ...
    const bestScore = stageProgress.length > 0 
      ? Math.max(...stageProgress.map(p => p.score)) 
      : 0;
    return {
      name: `م${stage.id}`,
  ...
  const chartConfig = {
    ...
    required: { label: 'المطلوب', color: 'hsl(var(--muted))' }
  };
                    ...
                    return (
                      <motion.div
                        ...
                        className={`flex items-center gap-4 p-4 rounded-xl ${
                          p.is_completed ? 'bg-success/10' : 'bg-muted/50'
                        }`}
                      >
